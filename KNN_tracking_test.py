...
class Car():
    ...
    def update(self,img, t):
        ...
        if ok:
            ...
            # cv2.putText(img, str(self.id), p1, cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 0, 0), 1, cv2.LINE_AA)
            for j in range(len(temp)):
                for i, v in enumerate(roi[0]):
                    if i < len(roi[0]) - 1:
                        img = cv2.line(img, tuple(temp[j][i]), tuple(temp[j][i + 1]), (0, 0, 255), 2)
                    else:
                        img = cv2.line(img, tuple(temp[j][i]), tuple(temp[j][0]), (0, 0, 255), 2)
...
if __name__ == '__main__':
    ...
    # Read video
    cap = cv2.VideoCapture("C:/Users/user/Desktop/영상/인코딩/F20010_6_202011011930.avi")
