...
# Video source
inputFile = loc+'/inputs/800_202004301300.mp4'
...
class VehicleCounter(object):
    ...
    def update_count(self, matches, output_image = None):
        ...
        if output_image is not None:
            ...
            # LHS
            cv2.putText(output_image, ("LH Lane: %03d" % self.vehicle_LHS), (12, 56)
                , cv2.FONT_HERSHEY_PLAIN, 1.2, (127,255, 255), 2)
            # RHS
            cv2.putText(output_image, ("RH Lane: %03d" % self.vehicle_RHS), (216, 56)
                , cv2.FONT_HERSHEY_PLAIN, 1.2, (127, 255, 255), 2)
...
CONTOUR_WIDTH = 21 #자동차를 탐지하기 위한 폭
CONTOUR_HEIGHT = 26#자동차를 탐지하기 위한 높이 # 40이면 456파일에서 경차 탐색 안
...
DEFAULT_AVERAGE_WEIGHT = 0.05#0.01 #작을 수록 차량이 잘 감지됨
...
#                      ", h=",h,") valid=",contour_valid)
        ...
        if car_counter is None:
            ...
            car_counter = VehicleCounter(frame.shape[:2], 7*frame.shape[0] / 10)
