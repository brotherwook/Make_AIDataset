...
def makeXML(id, name, blobs):
    ...
    image = root.find('track[@id="' + str(id) + '"]')
    if image is None:
    ...
    for i in range(len(blobs)):
        ...
        xtl = blobs[i][0] - half_width
        ytl = blobs[i][1] - half_height
        xbr = blobs[i][0] + half_width
        ybr = blobs[i][1] + half_height
...
def compare(clicks, blobs):
    ...
    for i, click in enumerate(clicks):
        for j, center in enumerate(blobs):
            if (click[0] - margin < center[0] < click[0] + margin) and (
                    click[1] - margin < center[1] < click[1] + margin):
        ...
        if t % int(cap.get(cv2.CAP_PROP_FPS)) == 0:
            while True:
                ...
                for i, center in enumerate(blobs):
                    cv2.rectangle(clone, (center[0] - half_width, center[1] - half_height),
                                  (center[0] + half_width, center[1] + half_height), (255, 255, 255), 2)
